...
const File = ({file}) => {
    ...
    if(filesView==='plate'){
        return (
            <Tooltip placement="bottom" title={file.type === 'dir' ? `Название: ${file.name} Дата: ${file.date.slice(0, 10)}`: `Название: ${file.name} Размер: ${sizeFormat(file.size)} Дата: ${file.date.slice(0, 10)}`} >
